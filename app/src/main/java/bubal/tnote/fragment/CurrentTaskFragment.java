...
import bubal.tnote.adapter.CurrentTasksAdapter;
import bubal.tnote.database.DBHelper;
import bubal.tnote.model.ModelSeparator;
    ...
    public void addTask(ModelTask newTask, boolean saveToDB) {
        {
            ...
            //Automatically sort new tasks by date
            ...
            //Selecting needed separators
            if (newTask.getDate() == 0) {
                newTask.setDateStatus(ModelSeparator.TYPE_NO_DATE);
                if (!adapter.containsSeparatorNoDate) {
                    adapter.containsSeparatorNoDate = true;
                    separator = new ModelSeparator((ModelSeparator.TYPE_NO_DATE));
                }
                ...
                if (!adapter.containsSeparatorTomorrow) {
                    ...
                    && calendar.get(Calendar.DAY_OF_YEAR) <= Calendar.getInstance().get(Calendar.DAY_OF_YEAR) + 7) {
                newTask.setDateStatus(ModelSeparator.TYPE_NEXT_SEVEN_DAYS);
                if (!adapter.containsSeparatorNextSevenDays) {
                    adapter.containsSeparatorNextSevenDays = true;
                    separator = new ModelSeparator(ModelSeparator.TYPE_NEXT_SEVEN_DAYS);
                }
            } else if (calendar.get(Calendar.DAY_OF_YEAR) > Calendar.getInstance().get(Calendar.DAY_OF_YEAR) + 7) {
            ...
            //To prevent incorrect placing of tasks inside separators
            if (position != -1) {
